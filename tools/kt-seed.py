...
ARCH = [("kitty", "nvim src/kt_server.py", 900), ("firefox", "github.com/kuhyx", 600),
        ("claude", "Claude Code", 1500), ("Slack", "#eng", 300), ("kitty", "zsh", 240),
        ("firefox", "awesome-selfhosted", 420), ("obsidian", "notes", 300)]
PIXEL = [("com.android.chrome", "", 300), ("org.thoughtcrime.securesms", "", 180),
         ("com.google.android.youtube", "", 900), ("com.spotify.music", "", 600),
...
def gen(device, apps, day_start, awake_hours):
    ...
    for bid, t, c in ((win, "currentwindow", "kt-seed"), (afk, "afkstatus", "kt-seed")):
        call(f"/api/0/buckets/{bid}", {"type": t, "client": c, "hostname": device + SUF,
                                       "device": device + SUF})
    ...
    while t < endt:
        ...
        evs.append({"timestamp": t.isoformat(), "duration": dur,
                    "data": {"app": app, "title": title} if not app.startswith("com.")
                    else {"app": app, "package": app, "classname": ".Main", "title": ""}})
        afks.append({"timestamp": t.isoformat(), "duration": dur, "data": {"status": "not-afk"}})
        t += timedelta(seconds=dur)
        if random.random() < 0.15:  # a break
            g = random.randint(300, 2400)
            afks.append({"timestamp": t.isoformat(), "duration": g, "data": {"status": "afk"}})
...
for d in range(a.days):
    day = datetime.combine((datetime.now().astimezone() - timedelta(days=d)).date(),
                           dtime.min).astimezone()
    total += gen("arch", ARCH, day, (9, 19))
    total += gen("pixel6a", PIXEL, day, (7.5, 23))
print(f"seeded {total} events over {a.days} day(s) -> {URL}  (buckets suffixed '{SUF or '(real)'}')")
